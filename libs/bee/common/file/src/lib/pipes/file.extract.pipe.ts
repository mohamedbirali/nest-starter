...
import { IHelperFileRows } from '@bee/common/helper'; ///interfaces/helper.interface';
import { HelperFileService } from '@bee/common/helper'; ///services/helper.file.service';
  ...
  async transform(
    ...
    if (Array.isArray(value)) {
      ...
      for (const val of value) {
        ...
        const extract: IFileExtractAllSheets<T> = await this.extract(val);
        extracts.push(extract);
      }
